...
Server-side Java web development, structured almost universally around
the Model--View--Controller pattern~\cite{krasner1988}, is dominated by
a small number of mature frameworks that occupy distinct points along
two axes: how much they bundle, and when they do their work (at runtime
versus at compile/build time).
...
frameworks first-class targets for GraalVM native-image~\cite{graalvm,wimmer2019}
...
bundled by default or added by the developer. This convenience carries
a cost that has become increasingly visible: transitive-dependency
trees are frequently bloated~\cite{sotovalero2021}, and the open-source
supply chain they form is itself an attack surface, as a growing body
of documented incidents attests~\cite{ohm2020}. Security in particular
is predominantly \emph{opt-in}: a separate module (e.g.\ a security
...
protections---dates back to the classic design principles of Saltzer
and Schroeder~\cite{saltzer1975} (``fail-safe defaults'') and is
reaffirmed in modern practitioner guidance such as the OWASP
Top~10~\cite{owasp}. In
